...
class Searchbar(Gtk.HBox):
    ...
    def _search_compile(self, searchtext):
        """Compile searchtext into a Python regex, honouring Match Case"""
        flags = 0
    ...
    def _do_search_custom(self, searchtext):
        ...
        if not self.search_is_inverted:
            ...
            found = self._find_match(start[0], start[1], True, self.wrap.get_active())
        else:
            start = (int(adj.get_value() + adj.get_page_size()) - 1, 1 << 30)
            found = self._find_match(start[0], start[1], False, self.wrap.get_active())
        if found:
    ...
    def next_search(self, widget):
        ...
        if self._custom_active():
            found_result = self._advance(True)
        else:
    ...
    def prev_search(self, widget):
        ...
        if self._custom_active():
            found_result = self._advance(False)
        else:
    ...
    def _advance(self, forward):
        ...
        if self._current is not None:
            ...
            if forward:
                ...
                start_col = 1 << 30
        found = self._find_match(row, start_col, forward, self.wrap.get_active())
        if found:
            self._set_current(found)
        return bool(found)
